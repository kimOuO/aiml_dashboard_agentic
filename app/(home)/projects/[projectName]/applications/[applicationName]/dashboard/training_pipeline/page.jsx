...
export default function Training_Pipeline_Page() {
  const {projectName, applicationName} = useParams();
  const handleBackClick = useBackNavigation();
  const projectNameDecode = decodeURIComponent(projectName);
  const applicationNameDecode = decodeURIComponent(applicationName);
  const {trainingPipelines, isLoading} = useFetchTrainingPipeline(projectNameDecode,applicationNameDecode)
  const {handleModelClick, handlePreprocessingPipelineClick} = handleLinkClick(projectNameDecode,applicationNameDecode);
  return (
